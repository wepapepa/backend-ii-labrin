...
export default class CartServices extends Services {
    ...
    addProdToCart = async (cartId, prodId) => {
        try {
            ...
            return await this.addProdToCart.addProdToCart(cartId, prodId);
        } catch (error) {
    ...
    removeProdToCart = async (cartId, prodId) => {
        try {
            ...
            const existProdInCart = await this.addProdToCart.existProdInCart(cartId, prodId);
            if (!existProdInCart) return null;

            return await this.addProdToCart.removeProdToCart(cartId, prodId);
        } catch (error) {
            throw new Error(error);
